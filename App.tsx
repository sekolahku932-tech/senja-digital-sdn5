...
function App() {
  ...
  const userGrade = isStudent ? (session as Student).grade : null;
  const teacherClass = !isStudent && role === 'teacher' ? (session as User).assignedClass : null;

  // We don't need explicit refreshData() anymore because Firebase is realtime!
  // But we keep empty handlers for prop compatibility or specific actions
...
const Dashboard = ({ role, stats }: { role: string, stats: any }) => (
...
const StudentManager = ({ students, role, assignedClass }: any) => {
    ...
    // Filter students
    const displayStudents = role === 'teacher' 
        ? students.filter((s: Student) => s.grade === assignedClass)
        : students;

    const handleSave = async (e: React.FormEvent) => {
        e.preventDefault();
        const student: Student = {
            id: editMode ? formData.id! : '', // Empty ID for new, will be generated by Firebase
            nisn: formData.nisn!,
            name: formData.name!,
...
const MaterialManager = ({ materials, role, assignedClass }: any) => {
    const [viewMode, setViewMode] = useState<'list' | 'form'>('list');
    const [formData, setFormData] = useState<Partial<Material>>({ questions: [] });
    const [inputType, setInputType] = useState<'url' | 'file'>('url');
    
    const displayMaterials = role === 'teacher' 
        ? materials.filter((m: Material) => m.grade === assignedClass)
        : materials;
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid md:grid-cols-2 gap-4">
                {displayMaterials.map((m: Material) => (
...
const GradingManager = ({ submissions, materials, role, assignedClass, onUpdate }: any) => {
    const [selectedSub, setSelectedSub] = useState<Submission | null>(null);

    // Filter logic
    const displaySubs = role === 'teacher' 
        ? submissions.filter((s: Submission) => s.grade === assignedClass)
        : submissions;
    ...
    if (selectedSub) {
        const material = materials.find((m: Material) => m.id === selectedSub.materialId);
        return (
                        ...
                        <div>
                            ...
                            <ul className="space-y-4">
                                {selectedSub.answers.map((ans, i) => {
                                    const qText = material?.questions.find(q => q.id === ans.questionId)?.text || `Pertanyaan ${i+1}`;
                                    return (
...
const SettingsManager = ({ settings, onSave }: any) => {
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        const file = e.target.files?.[0];
        if (file) {
            const reader = new FileReader();
